 ...
 *
 * Dry-run for transport-only withdrawal. Same shape as the commit endpoint
 * but no writes. UI calls this on every effective-date change.
 */
...
export async function POST(
  ...
  try {
    ...
    const allocation = await db.transportAllocation.findFirst({
      ...
      select: { id: true, academicYear: true, stopName: true, fareAmount: true },
    })
    if (!allocation) {
      return NextResponse.json({
        success: true,
        canCommit: false,
        blockers: ['No active transport allocation to withdraw'],
        cancelledItems: [],
        cancelledAmount: 0,
        requiresRefund: [],
        totalRefundDue: 0,
    ...
    console.error('POST /students/[id]/transport/withdraw/preview failed:', error)
    return internalError('Failed to preview transport withdrawal')
  }
}
