// One-time backfill: ensure the `transport:annual-setup` permission exists in
// the Permission catalog, and is granted to every existing School via
// SchoolPermission so that School Admins can use the Annual Transport Setup
// feature. Also links the permission to each school's "School Admin" role.
//
// Idempotent: safe to re-run. Skips records that already exist.
// Run with: bun run scripts/backfill-transport-annual-setup-permission.ts

import { db } from '../src/lib/db'

async function main() {
  const code = 'transport:annual-setup'
  const name = 'Annual Transport Setup'
