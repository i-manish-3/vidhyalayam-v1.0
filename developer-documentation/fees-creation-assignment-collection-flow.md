...
8. **Receipt cancellation is supported.** Users with `fees:refund` can cancel a fee receipt from **Fees → Fee Receipts** with a mandatory reason. The cancellation transaction soft-voids the payment CREDIT and same-receipt waivers, releases their allocations, rebuilds affected DEBIT/legacy collection/invoice balances, and writes a `payment_cancelled` audit entry. Cancelled payments are excluded from parent receipts and fee-report payment counts.
